...
pub(super) fn spawn_workers(
    ...
    request_rx: Arc<Mutex<std::sync::mpsc::Receiver<S3Request>>>,
    ...
    for _ in 0..worker_threads {
        ...
        let handle = thread::spawn(move || {
            let runtime = tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("failed to build tokio runtime for S3 worker");
            loop {
                let request = {
                    let receiver = worker_request_rx
                        .lock()
                        .expect("failed to lock S3 request receiver");
                    receiver.recv()
                };
                match request {
                    Ok(S3Request::Put { name, key, data }) => {
                        debug!("Uploading object to S3: {}", name);
                        let result = runtime.block_on(async {
                            worker_client
                                .put_object()
                                .bucket(worker_bucket.as_str())
                                .key(key)
                        ...
                        let _ = worker_result_tx.send(S3Result::Put { name, result });
                    }
                    Ok(S3Request::Get { name, key }) => {
                        debug!("Fetching object from S3: {}", name);
                        let result = runtime.block_on(async {
                            let output = worker_client
                                .get_object()
                                .bucket(worker_bucket.as_str())
                                .key(key)
                                .send()
                        ...
                        let _ = worker_result_tx.send(S3Result::Get { name, result });
                    }
                    Ok(S3Request::List { respond_to }) => {
                        let result = runtime.block_on(async {
                            list_objects_async(&worker_client, &worker_bucket, &worker_prefix).await
                        });
