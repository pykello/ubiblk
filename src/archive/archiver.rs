...
impl StripeArchiver {
        ...
        for (stripe_id, success) in completed {
            if !success {
                return Err(crate::UbiblkError::ArchiveError {
                    description: format!(
                        "I/O error while fetching stripe {} from block device",
                        stripe_id
                    ),
                });
            }

            debug!("Completed fetching stripe {} from block device", stripe_id,);
    ...
    fn start_upload_stripe(&mut self, stripe_id: usize) -> Result<()> {
        let stripe_state = self
            .stripe_states
            .get(&stripe_id)
            .expect("Stripe state should exist when storing stripe");
    ...
    fn poll_uploads(&mut self) -> Result<()> {
        ...
        for (obj_name, result) in results {
            ...
            let stripe_state = self
                .stripe_states
                .remove(&stripe_id)
                .expect("Stripe state should exist after upload");
