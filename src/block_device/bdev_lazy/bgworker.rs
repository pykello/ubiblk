...
use crate::Result;
use crate::{block_device::BlockDevice, stripe_source::StripeSource};
use log::{error, info};
...
mod tests {
    ...
    fn build_bg_worker() -> (BgWorker, std::sync::mpsc::Sender<BgWorkerRequest>) {
        ...
        let stripe_source =
            Box::new(stripe_source::BlockDeviceStripeSource::new(&source_dev, 16).unwrap());
        init_metadata(
    ...
    fn bg_worker_supports_null_source() {
        ...
        let metadata_dev = TestBlockDevice::new(1024 * 1024);
        let stripe_source =
            Box::new(stripe_source::BlockDeviceStripeSource::new(source_dev.as_ref(), 16).unwrap());
        init_metadata(
