...
jitter_ms)` — a fixed floor plus jitter, rather than exponential backoff.
...
It applies to every S3 operation of the client it is set on (for an archive
target, mostly PUTs). Only responses with an HTTP status are delayed — client-side
